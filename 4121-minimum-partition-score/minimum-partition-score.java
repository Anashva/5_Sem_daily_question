class Solution {
    ...
    static class Line {
        ...
        long value(long x) { return m * x + c; }
    }
    ...
    private void addLine(Deque<Line> hull, long m, long c) {
        ...
        if (hull.isEmpty()) newLine.xLeft = Double.NEGATIVE_INFINITY;
        hull.addLast(newLine);
    }

    private long query(Deque<Line> hull, long x) {
        while (hull.size() >= 2) {
            Line first = hull.peekFirst();
            Line second = hull.toArray(new Line[0])[1];
            if (second.xLeft <= x) hull.pollFirst();
            else break;
        }
